...
class CLIPLoss(torch.nn.Module):
    def __init__(self, device, lambda_direction=1., lambda_patch=0., lambda_global=0., lambda_manifold=0.,
        ...
        model_cnn, preprocess_cnn, _ = clip_lib.create_model_and_transforms('RN50', pretrained='openai')
        # Convert to float16 to match SD precision
        device_str = str(self.device)
        if 'cuda' in device_str:
            model_cnn = model_cnn.half()
        self.model_cnn = model_cnn.to(self.device)
    ...
    def tokenize(self, strings: list):
        return clip_lib.tokenize(strings).to(self.device)
    ...
    def get_text_features(self, class_str: str, templates=imagenet_templates, norm: bool = True) -> torch.Tensor:
        ...
        tokens = clip_lib.tokenize(template_text).to(self.device)
    ...
    def global_clip_loss(self, img: torch.Tensor, text) -> torch.Tensor:
        ...
        tokens = clip_lib.tokenize(text).to(self.device)
        image = img.float()

        try:
            result = self.model(image, tokens)
        except Exception:
    ...
    def patch_scores(self, img: torch.Tensor, class_str: str, patch_centers, patch_size: int) -> torch.Tensor:
        ...
        tokens = clip_lib.tokenize(parts).to(self.device)
        text_features = self.encode_text(tokens).detach()
